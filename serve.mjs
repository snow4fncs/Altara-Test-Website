...
const reviews = [
  // ── Midnight Black ────────────────────────────────────────────────────────
  {
    ...
    verified: true, approved: true, photos: [],
  },
  {
  ...
  // ── One left unpublished so the moderation queue is not empty locally ─────
  {
    ...
    verified: true, approved: false, photos: [],
  },
];
...
async function handleReviews(req, res, query) {
  ...
  if (req.method === 'POST') {
    ...
    const nPh = Array.isArray(photos) ? photos.slice(0,3).length : 0;
    const kb = Math.round((Array.isArray(photos)?photos.slice(0,3):[]).reduce((n,d)=>n+String(d).length*0.75,0)/1024);
    console.log(`  → review submitted for ${prod} (${stars}★, ${nPh} photo(s) ${kb}KB) — pending in /admin-reviews.html`);
    return send(res, 200, { success: true });
  }
...
let mockOrders = [
  { id:'o1', created_at:'2026-08-13T03:43:00Z', customer_email:'yahyashahid99@gmail.com', customer_name:'Yahya Shahid',
    ...
    shipped_at:'2026-08-21T02:00:00Z', shipped_email_at:'2026-08-21T02:05:00Z', review_email_at:null, repeat_email_at:null },
  { id:'o2', created_at:'2026-08-13T11:53:00Z', customer_email:'brucecrayton@hotmail.com', customer_name:'Bruce Crayton',
    ...
    shipped_at:'2026-08-08T02:00:00Z', shipped_email_at:null, review_email_at:null, repeat_email_at:null },
